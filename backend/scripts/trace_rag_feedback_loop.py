...
def run_rag_feedback_loop_verification():
    ...
    print("=" * 85)
    # 1. Run trace in an isolated ephemeral sandbox so permanent ChromaDB on disk is completely untouched
    with tempfile.TemporaryDirectory() as temp_chroma:
        ...
        print(f"[*] Initial ChromaDB sandbox count: {initial_count} (Cold Start)")
        ...
        for idx, item in enumerate(batch_definitions, 1):
            ...
            # Create test transaction & failure
            tx = Transaction(
                id=f"tx_trace_{idx:02d}",
                customer_id=cust.id,
                amount=amt,
                currency="INR",
            ...
            # Fetch detailed case state from service
        ...
        novel_proposal = ProposedRecoveryAction(
            ...
            retry_delay_hours=1,
            confidence=0.50,
            empirical_confidence=0.50,
            retrieved_precedent_count=1,
            insufficient_precedent=True,
            reasoning="Grounded fallback for completely unseen transaction profile.",
        )
